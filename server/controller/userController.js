...
const coverUploder = async (req, res) => {
  ...
  try {
    const respose = await people.findByIdAndUpdate(
      { _id: req.params.id },
      {
        $set: {
...
const profilePicUploder = async (req, res) => {
  try {
    ...
    if (respose) {
      ...
      const statusUserUpdate = await statusModel.updateMany(
      ...
      console.log(statusUserUpdate)

      const commentUserUpdate = await commentModel.updateMany(
        { "user.id": req.user._id },
        {
          $set: {
            "user.avater": req.file.filename,
      ...
      const replyUserUpdate = await repliesModel.updateMany(
        { "user.id": req.user._id },
        {
          $set: {
            "user.avater": req.file.filename,
